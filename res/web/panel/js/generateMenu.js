(() =>{
	...
	function generateMenu(dictm,ElementIdName){
		...
		for(let i = 0;i < dictm.length;i++){
			const ulx_li = $('<li class="centerx_menu_item items_root"></li>');
			const item_childs = createEleAddClass("div","item-childs");
			const item_title = generateItemTitle(dictm[i].name,dictm[i].icoClass);
			
			const childs = dictm[i].child;
			if(childs.length>0){
				item_title.parents = ulx_li;
				item_title.onclick = show_child;
				
				const ulxtemp = document.createElement('ul');
				for(let i = 0;i < childs.length;i++){
					const ulx_litemp = createEleAddClass("li","centerx_menu_item");
					const item_titletemp = generateItemTitle(childs[i].name,childs[i].icoClass);
	...
	function generateItemTitle(name,ico_class){
		const item_title = createEleAddClass("div","item-title");
