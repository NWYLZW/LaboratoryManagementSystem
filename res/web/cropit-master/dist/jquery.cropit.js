	...
	([
		...
		function(module, exports, __webpack_require__) {
			...
			_jquery2['default'].fn.cropit = function(method) {
				...
				} else if (['imageState', 'imageSrc', 'offset', 'previewSize', 'imageSize', 'zoom', 'initialZoom', 'exportZoom',
						'minZoom', 'maxZoom'
					].indexOf(method) >= 0) {
			...
			var Cropit = (function() {
				...
				_createClass(Cropit, [{
					...
					value: function init() {
						...
						this.$fileInput = this.options.$fileInput.attr({ accept: 'image/png,image/jpeg' });
						...
						this.$image = (0, _jquery2['default'])('<img />').addClass(_constants.CLASS_NAMES.PREVIEW_IMAGE).attr(
							'alt', '').css({
							transformOrigin: 'top left',
							webkitTransformOrigin: 'top left',
							willChange: 'transform'
						});
						this.$imageContainer = (0, _jquery2['default'])('<div />').addClass(_constants.CLASS_NAMES.PREVIEW_IMAGE_CONTAINER)
							.css({
								position: 'absolute',
								overflow: 'hidden',
								left: 0,
						...
						if (this.options.imageBackground) {
							...
							this.$bg = (0, _jquery2['default'])('<img />').addClass(_constants.CLASS_NAMES.PREVIEW_BACKGROUND).attr(
								'alt', '').css({
								position: 'relative',
							...
							this.$bgContainer = (0, _jquery2['default'])('<div />').addClass(_constants.CLASS_NAMES.PREVIEW_BACKGROUND_CONTAINER)
								.css({
									position: 'absolute',
									zIndex: 0,
									top: -this.bgBorderWidthArray[0],
					...
					value: function renderImage() {
						var transformation = '\n      translate(' + this.rotatedOffset.x + 'px, ' + this.rotatedOffset.y +
					...
					value: function shouldRejectImage(_ref) {
						...
						return imageWidth * maxZoom < previewSize.width * exportZoom || imageHeight * maxZoom < previewSize.height *
							exportZoom;
					}
				}, {
					...
					set: function(newZoom) {
						...
						if (this.imageLoaded) {
							...
							var newY = this.previewSize.height / 2 - (this.previewSize.height / 2 - this.offset.y) * newZoom /
								oldZoom;
					...
					get: function() {
						return {
							x: this.offset.x + (this.rotation === 90 ? this.image.height * this.zoom : 0) + (this.rotation === 180 ?
								this.image.width * this.zoom : 0),
							y: this.offset.y + (this.rotation === 180 ? this.image.height * this.zoom : 0) + (this.rotation === 270 ?
								this.image.width * this.zoom : 0)
						};
					}
				}, {
